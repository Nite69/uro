...
uint256 hashGenesisBlock("0x000001196bbe430b7e0cdce3504f5ddfda0d0313ea479526d79afbf4d090a880");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Uro: starting difficulty is 1 / 2^12
...
{
    // Uro: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // Uro: 24 hours
static const int64 nTargetSpacing = 3 * 60; // Uro: 3 minutes
...
{
        static const int64 BlocksTargetSpacing = 3 * 60; // Uro: 3 minutes
...
{
    // Uro: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // Uro: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UroMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UroMiner : ProcessBlock, block not accepted");
    }
